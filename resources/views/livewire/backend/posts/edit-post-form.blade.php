<div>
    <div class="card">
        <div class="card-header">
            <h3 class="card-title"><i class="fa fa-newspaper me-2"></i>İçerik Düzenle</h3>
            @can('posts_create')
        ...
        <div class="card-body row">
            <div class="col-12 col-lg-6">
                ...
                <div id="content_container" wire:ignore>
                    <textarea wire:model.lazy="content" name="content" id="content" cols="30" rows="4" style="height: 400px">
                        {!! Request::old('content', $content) !!}
                    </textarea>
                </div>
            ...
            <div class="col-lg-6">
                ...
                <div class="mb-3">
                    <div class="form-label"><strong>İçerik Fotoğrafı</strong>
                        @if($thumb_updated || $existing_featured_photo)
                ...
                @if($is_featured)
                    @if($photo==NULL && !$existing_featured_photo)
                ...
                @if(count($existingFiles)>0 || !empty($attached_files))
                    <div class="row pt-2 ps-2 pe-1">
                        <table class="table">
                            ...
                            @foreach($existingFiles as $file)
                                <tr>
                                    <td>{{$file->name}} <span class="badge badge-sm bg-success">Mevcut</span></td>
                                    <td>{{$file->custom_properties["extension"]}}</td>
                                    <td>{{$file->humanReadableSize}}</td>
                                    <td><a class="btn btn-square btn-success" wire:click="downloadFile({{$file->id}})" wire:loading.attr="disabled" wire:target="removeFile"><i class="fa fa-download"></i></a></td>
                                    <td><button class="btn btn-square btn-danger" wire:click="removeFile({{$file->id}})" wire:loading.attr="disabled" wire:target="removeFile"><i class="fa fa-trash"></i></button></td>
                                </tr>
                            @endforeach
                            @if(!empty($attached_files))
                                @foreach($attached_files as $key => $attached_file)
                                    <tr>
                                        <td>{{$attached_file['filename']}} <span class="badge badge-sm bg-danger">Yeni</span></td>
                                        <td>{{$attached_file['extension']}}</td>
                                        <td>{{$attached_file['file_size']}} MB</td>
                                        <td></td>
                                        <td><button class="btn btn-square btn-danger" wire:click="detachFile({{$key}})" wire:loading.attr="disabled" wire:target="detachFile"><i class="fa fa-trash"></i></button></td>
                                    </tr>
                                @endforeach
                            @endif
                            </tbody>
                        ...
                        @if(!empty($attached_files))
                            <div class="alert alert-warning">
                                Henüz kaydedilmemiş dosyalarınız var.
                            </div>
                        @endif
                    </div>
                @endif
        ...
        <div class="card-footer text-end">
            @if(count($existingFiles)>1)
                <button class="btn btn-primary" wire:click="downloadAllFiles({{$post->id}})"><i class="fa fa-file-zipper me-2"></i>Ekleri İndir</button>
            @endif
