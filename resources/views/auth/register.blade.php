...
@section('content')
    <form class="card card-md" action="{{ route('register') }}" method="post" autocomplete="off">
        ...
        <div class="card-body">
            <h2 class="card-title text-center mb-4">{{ __('Hesap Oluştur') }}</h2>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="surname" class="form-control @error('surname') is-invalid @enderror" placeholder="{{ __('Soyad') }}">
                @error('surname')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
